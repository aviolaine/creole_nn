...
class StackedRNNModel(nn.Module):
    ...
    def init_weights(self,embeds,ntoken,ninp):
        ...
        if hidden is not None:
            #print("pls")
            self.rnn_bottom.weight_ih_l0 = hidden.rnn.weight_ih_l0
            self.rnn_bottom.weight_hh_l0 = hidden.rnn.weight_hh_l0
            self.rnn_bottom.bias_ih_l0 = hidden.rnn.bias_ih_l0
            self.rnn_bottom.bias_hh_l0 = hidden.rnn.bias_hh_l0
            self.rnn_bottom.weight_ih_l1 = hidden.rnn.weight_ih_l1
            self.rnn_bottom.weight_hh_l1 = hidden.rnn.weight_hh_l1
            self.rnn_bottom.bias_ih_l1 = hidden.rnn.bias_ih_l1
            self.rnn_bottom.bias_hh_l1 = hidden.rnn.bias_hh_l1
            #print("plsss")
    ...
    def forward(self, input, hidden):
        ...
        i = 0
        for em in emb:
            emb[i] = torch.cat((emb[i],mult[i]))
            i+=1
        
        #reset hidden or no?
        output, hidden = self.rnn_top(emb, hidden)
        output = self.drop(output)
        decoded = self.decoder(output.view(output.size(0)*output.size(1), output.size(2)))
        return decoded.view(output.size(0), output.size(1), decoded.size(1)), hidden
